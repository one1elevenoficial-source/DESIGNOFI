...
export default function Inbox() {
  ...
  const currentUser = { name: profile?.full_name || 'Usuário' };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const filePath = `${workspace_id}/${selectedConversation.id}/${fileName}`;
    ...
    try {
      ...
      await supabase.from('messages').insert({
        ...
        direction: 'out',
        type: fileType,
        body: publicUrlData.publicUrl
      });
      
      qc.invalidateQueries({ queryKey: ['messages', selectedConversation.id] });
    } catch (err) {
      console.error('Upload falhou', err);
    }
  };

  const handleDropdownAction = async (action: 'resolved' | 'transfer' | 'profile' | 'muted', leadId: string) => {
    if (action === 'profile') {
      navigate(`/leads/${leadId}`);
      return;
    }
    try {
      if (action === 'resolved' || action === 'muted') {
        await supabase.from('leads').update({ conversation_status: action }).eq('id', leadId);
      } else if (action === 'transfer') {
        await supabase.from('leads').update({ 
          human_attending: true, 
          human_attending_by: currentUser.name, 
        ...
        await supabase.from('circuit_breaker').upsert({ 
          workspace_id, 
          lead_id: leadId, 
        ...
        <ScrollArea className="flex-1">
          <div className="p-2">
            ...
            {filteredConversations.map((conv) => (
              <div
                key={conv.id}
                onClick={() => navigate(`/leads/${conv.id}`)}
                className={cn(
            ...
            <div className="flex items-center gap-2">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="bg-popover border-border">
                  <DropdownMenuItem onClick={() => handleDropdownAction('resolved', selectedConversation.id)}>
                    Marcar como resolvido
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleDropdownAction('transfer', selectedConversation.id)}>
                    Transferir para humano
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleDropdownAction('profile', selectedConversation.id)}>
                    Ver perfil
                  </DropdownMenuItem>
                  <DropdownMenuSeparator className="bg-border" />
                  <DropdownMenuItem onClick={() => handleDropdownAction('muted', selectedConversation.id)} className="text-destructive focus:bg-destructive focus:text-destructive-foreground">
                    Silenciar
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            ...
            <div className="flex items-end gap-2">
              ...
              <Button 
                ...
                onClick={() => {
                  ...
                  setTimeout(() => fileInputRef.current!.setAttribute('accept', 'image/*,audio/*'), 1000);
                }}
              >
